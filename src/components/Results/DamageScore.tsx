    ...
    <SectionCard title="Estimated Damage Score" icon={<Zap size={15} />}>
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3 bg-orange-500/10 border border-orange-500/20 rounded-lg px-4 py-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">
              Optimal ratio is {improvement > 0 ? "better" : "equal to"} a naive
              33/33/33 split for your stats.
            </p>
          </div>
        </div>
        ...
        <div className="text-xs text-gray-500 border-t border-gray-800 pt-3 flex items-center justify-between">
          ...
          <span>
            Participants:{" "}
            <strong className="text-gray-300">{config.participants}</strong>
          </span>
          <span>
            Fill:{" "}
            <strong className="text-gray-300">
              {(
                ((Math.floor(config.capacity / config.participants) *
                  config.participants) /
                  config.capacity) *
                100
              ).toFixed(1)}
        ...
        <p className="text-xs text-gray-600 italic">
          Score is relative and proportional to the damage formula. Absolute
          damage depends on rally size and base attack stats.
        </p>
      </div>
    </SectionCard>
  );

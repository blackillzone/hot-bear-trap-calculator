...
const JOINER_OPTIONS = JOINER_HEROES.map((n) => ({
  value: n,
  label: n === "None" ? "— None —" : n,
}));
    ...
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          >
            <NumberInput
              ...
              onChange={(v) =>
                setRallyConfig({ capacity: Math.max(1000, Math.round(v)) })
              }
              min={10_000}
              max={50_000_000}
              step={100_000}
          ...
          >
            <div className="space-y-1">
              <input
                ...
                onChange={(e) =>
                  setRallyConfig({ participants: parseInt(e.target.value, 10) })
                }
          ...
          >
            <Select<0 | 1 | 2 | 3 | 4 | 5>
              ...
              onChange={(v) =>
                setRallyConfig({ bearLevel: v as 0 | 1 | 2 | 3 | 4 | 5 })
              }
      ...
      <SectionCard title="Joiner Heroes" icon={<UserPlus size={15} />}>
        <p className="text-xs text-gray-500 mb-3">
          Joiner heroes provide passive bonuses to{" "}
          <strong className="text-gray-400">all troops</strong>. Select each
          hero and their skill level. Recommended: Amane + Chenko + Yeonwoo.
        </p>
              ...
              <div key={slot} className="space-y-1.5">
                ...
                {joiner.hero !== "None" && (
                  <div className="space-y-1">
                    ...
                    />
                    <p
                      className={`text-xs px-1 ${hasBonus ? "text-green-400" : "text-gray-500"}`}
                    >
                      {bonusLabel}
                    </p>
                  </div>
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <span className="text-xs text-gray-500">
            ...
            <strong className="text-gray-300">
              {Math.floor(
                config.capacity / config.participants,
              ).toLocaleString()}
            </strong>
          </span>
        </div>

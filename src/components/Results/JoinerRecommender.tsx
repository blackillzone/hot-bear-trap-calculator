...
export function JoinerRecommender() {
  ...
  const recommendations = useMemo(() => {
    ...
    const scored = combos.map((combo) => {
      const joiners: JoinerSlot[] = combo.map((hero) => ({
        hero,
        skillLevel: 5,
      }));
  ...
  return (
    <SectionCard title="Joiner Recommendations" icon={<Lightbulb size={15} />}>
      <p className="text-xs text-gray-500 mb-3">
        Best combinations from {CANDIDATES.join(", ")}, ranked by predicted
        damage. Click a row to apply it.
      </p>
      ...
      <div className="space-y-2">
        {recommendations.map((rec, idx) => {
          ...
          return (
            ...
            >
              <span
                className={`font-bold w-5 text-center shrink-0 ${isSelected || idx === 0 ? "text-orange-400" : "text-gray-500"}`}
              >
                #{idx + 1}
              </span>
              <div className="flex-1 min-w-0">
                <p
                  className={`font-medium truncate ${isSelected || idx === 0 ? "text-white" : "text-gray-300"}`}
                >
                  {labelStr}
                </p>
                {bonusParts.length > 0 && (
              ...
              <span
                className={`shrink-0 font-semibold ${isSelected ? "text-orange-400" : idx === 0 ? "text-orange-400" : "text-gray-400"}`}
              >
                {rec.pct.toFixed(1)}%
              </span>
            </button>
      ...
      <p className="mt-3 text-xs text-gray-600">
        * Rankings use max skill level (5). Adjust levels in the Joiner Heroes
        section for exact scores.
      </p>
    </SectionCard>
  );
}

...
function WidgetCell({
  ...
  return (
    <Field
      label="Widget"
      hint="No exclusive gear expedition skill for rally on this hero"
    >
      <Select<string>
        value="none"
        onChange={() => {}}
...
export function StatsForm() {
  ...
  const handleWidgetLevelChange = (
    ...
    if (effect === "rally_atk")
      widgetUpdate[`${type}_atk` as keyof WidgetStats] = bonus;
    if (effect === "rally_let")
      widgetUpdate[`${type}_let` as keyof WidgetStats] = bonus;
    updateProfile({
      ...
      <SectionCard title="Rally Leader Stats" icon={<Zap size={15} />}>
        <div className="space-y-2 mb-4">
          ...
          <p className="text-xs text-gray-500 italic">
            Tip: Run a <strong className="text-gray-400">Terror rally</strong>{" "}
            with your bear setup and read ATK/LET stats from the battle report.
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-x-6 gap-y-2">
          ...
          <Field label="Attack %" hint="ATK% bonus from your stats">
            <NumberInput
              value={stats.inf_atk}
              onChange={(v) => handleStatChange("inf_atk", v)}
              suffix="%"
              min={0}
              max={5000}
            />
          ...
          <Field label="Attack %" hint="ATK% bonus from your stats">
            <NumberInput
              value={stats.cav_atk}
              onChange={(v) => handleStatChange("cav_atk", v)}
              suffix="%"
              min={0}
              max={5000}
            />
          ...
          <Field label="Attack %" hint="ATK% bonus from your stats">
            <NumberInput
              value={stats.arc_atk}
              onChange={(v) => handleStatChange("arc_atk", v)}
              suffix="%"
              min={0}
              max={5000}
            />
          ...
          <Field label="Lethality %" hint="LET% bonus from your stats">
            <NumberInput
              value={stats.inf_let}
              onChange={(v) => handleStatChange("inf_let", v)}
              suffix="%"
              min={0}
              max={5000}
            />
          ...
          <Field label="Lethality %" hint="LET% bonus from your stats">
            <NumberInput
              value={stats.cav_let}
              onChange={(v) => handleStatChange("cav_let", v)}
              suffix="%"
              min={0}
              max={5000}
            />
          ...
          <Field label="Lethality %" hint="LET% bonus from your stats">
            <NumberInput
              value={stats.arc_let}
              onChange={(v) => handleStatChange("arc_let", v)}
              suffix="%"
              min={0}
              max={5000}
            />
          ...
          {/* Row 4 — Widget */}
          <WidgetCell
            widgetEffect={infEffect}
            widgetLevel={wl.inf}
            onWidgetLevelChange={(l) => handleWidgetLevelChange("inf", l)}
          />
          <WidgetCell
            widgetEffect={cavEffect}
            widgetLevel={wl.cav}
            onWidgetLevelChange={(l) => handleWidgetLevelChange("cav", l)}
          />
          <WidgetCell
            widgetEffect={arcEffect}
            widgetLevel={wl.arc}
            onWidgetLevelChange={(l) => handleWidgetLevelChange("arc", l)}
          />
          ...
          <Field label="Lead Hero">
            <HeroSelect
              value={heroes.inf}
              onChange={(v) => handleHeroChange("inf", v)}
              options={LEAD_INF_HEROES}
            />
          </Field>
          <Field label="Lead Hero">
            <HeroSelect
              value={heroes.cav}
              onChange={(v) => handleHeroChange("cav", v)}
              options={LEAD_CAV_HEROES}
            />
          </Field>
          <Field label="Lead Hero">
            <HeroSelect
              value={heroes.arc}
              onChange={(v) => handleHeroChange("arc", v)}
              options={LEAD_ARC_HEROES}
            />
          </Field>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-5 pt-4 border-t border-gray-800">
          <Field
            label="Troop Tier"
            hint="Impacts the archer damage multiplier for T7+ with TG3+"
          >
            <Select<TroopTier>
              value={troop_tier}
          ...
          <Field
            label="TG Level"
            hint="Gilded level (TG3+ with T7+ gives extra archer bonus)"
          >
            <Select<TGLevel>
              value={tgLevel}

...
export function Field({ label, hint, children, className, row }: Props) {
  ...
  return (
    <div
      className={clsx(
        row ? "flex items-center gap-3" : "flex flex-col gap-1",
        className,
      )}
    >
      <label
        htmlFor={inputId}
        className={clsx(
          "text-xs font-medium text-gray-400 whitespace-nowrap",
          row && "w-28 shrink-0",
        )}
      >
        {label}
        {hint && (
          <span className="ml-1 text-gray-600 cursor-help" title={hint}>
            ⓘ
          </span>
        )}
      </label>
...
export function NumberInput({
  ...
  return (
    <div
      className={clsx(
        "flex items-center rounded-lg bg-gray-800 border border-gray-700 focus-within:border-orange-500 transition-colors",
        className,
      )}
    >
      <input

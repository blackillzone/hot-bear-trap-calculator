  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-xs shadow-xl">
      ...
      <p className="text-gray-400">
        Troops/player: {formatTroops(d.troopsPerParticipant)}
      </p>
      <p className="text-gray-400">
        Rally fill: {(d.fillRate * 100).toFixed(1)}%
      </p>
    </div>
  );
}
...
export function ParticipantGraph() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      <SectionCard
        title="Damage vs Participant Count"
        icon={<TrendingUp size={15} />}
      >
        <p className="text-xs text-gray-500 mb-4">
          More participants = more total troops = higher damage. But fewer
          participants = more troops per player. This graph shows the{" "}
          <strong className="text-gray-400">total estimated damage</strong> for
          each participant count, assuming the rally is filled with the optimal
          ratio.
        </p>
              ...
              <Bar dataKey="normalizedScore" radius={[3, 3, 0, 0]}>
                {normalized.map((entry) => (
                  <Cell
                    ...
                    fill={
                      entry.participants === config.participants
                        ? "#f97316"
                        : "#374151"
                    }
                  />
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-gray-500">
                  {formatTroops(d.troopsPerParticipant)}/ea
                </p>
              </button>
            );
          })}
